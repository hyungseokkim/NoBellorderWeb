...
function alertStr() {
	if (document.orderlist.update.value.length > 1) {
		alert("수정되었습니다.");
		return true;
	}
...
<body>
<h3>테이블번호:${order[0].table_number}</h3><br>
	<form:form id="orderlist" name="orderlist" method="post">
<input type="hidden" id="test" name="test" value="${order[0].table_number}">
					...
					<table align="center" width="95%" height="100%" border="0" cellpadding="0" cellspacing="0">
						...
						<c:forEach var="result" items="${order}" varStatus="status">
							<tr>
								<td align="center"><input type="hidden" id="table" name="table" value="${result.order_number}" />${result.order_number}</td>
								<td align="center">${result.menu_name}</td>
								<td align="center">${result.check_amount}</td>
								<td align="center">${result.check_price}</td>
								<td align="center">${result.check_discount}</td>
							</tr>
